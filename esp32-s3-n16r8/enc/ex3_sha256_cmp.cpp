...
// =========================================================
// 1. 純軟體 SHA-256 實作 (Mini Pure C)
// 為了教學對比，這裡手刻一個不使用硬體的版本
// =========================================================
...
void setup() {
  ...
  // 準備測試資料
  test_buffer = (uint8_t*)malloc(DATA_SIZE);
  for(int i=0; i<DATA_SIZE; i++) test_buffer[i] = (uint8_t)(i & 0xFF);

  Serial.println("\n\n===== ESP32 SHA-256 加速測試 =====");
  Serial.printf("資料大小: %d KB\n", DATA_SIZE / 1024);
  Serial.printf("測試次數: %d 次\n", TEST_ROUNDS);
  ...
  for(int i=0; i<TEST_ROUNDS; i++) {
    ...
    mbedtls_sha256_starts(&ctx_hw, 0); // 0 = SHA-256 (not 224)
    mbedtls_sha256_update(&ctx_hw, test_buffer, DATA_SIZE);
  ...
  Serial.println("--- [測試結果] ---");
  Serial.printf("硬體耗時: %.4f 秒 | 速度: %.2f MB/s\n", time_hw, total_mb / time_hw);
  Serial.printf("軟體耗時: %.4f 秒 | 速度: %.2f MB/s\n", time_sw, total_mb / time_sw);
  Serial.println("-------------------------");
  Serial.printf("加速倍率: %.2f 倍\n", time_sw / time_hw);
  
  // 印出最後一次的 Hash 以證明運算正確
  Serial.print("Hash Check (HW): ");
  for(int i=0; i<4; i++) Serial.printf("%02X", hash_result[i]);
  Serial.println("...");
}
